...
/* 
  RSA vectors examples for PKCS#1v1.5 are taken from:	
...
/******************************************************************************/
const uint8_t Message[] = {0xEB,0x39,0x49,0x2F,0x73,0xED,0x5E,0x1C,0x5E,
                           0x45,0xAF,0xB5,0x7F,0xC1,0xD6,0xFE,};

const uint8_t Modulus[] = {0xB5,0x05,0xFC,0xA2,0xCA,0x33,0x48,0xD5,0x9B,0xF3,0x00,0x5F,0x7C,0xFD,0xC4,0x56,0x4C,0x25,0x07,
...
const uint8_t PublicExponent[] = {0x01, 0x00, 0x01};

const uint8_t PrivateExponent[] = {0x06,0xBE,0x0F,0x57,0xDC,0xE2,0x26,0x1F,0x56,0xAC,0xA9,0x61,0xE5,0x1C,0xEA,0x98,0x30,0x43,
...
const uint8_t EncryptedMessage[] = {0x45,0x4E,0x4F,0xE2,0x40,0xBA,0xF4,0xD9,0xED,0xEA,0x65,0x79,0xB4,0xCF,0x8D,0xE4,0x41,0x3E,
  ...
  /* String of entropy */
	uint8_t entropy_data[32]={ 0x91, 0x20, 0x1a, 0x18, 0x9b, 0x6d, 0x1a, 0xa7,
...
int32_t STM32_RSA_Encrypt(RSApubKey_stt *P_pPubKey, 
                    const uint8_t *P_pInputMessage, 
                    int32_t P_InputSize, 
                    uint8_t *P_pOutput);
										
int32_t STM32_RSA_Decrypt(RSAprivKey_stt * P_pPrivKey, 
                    const uint8_t * P_pInputMessage,                     
                    uint8_t *P_pOutput,
...
{     
  ...
  /* Enable CRC clock */
   __CRC_CLK_ENABLE();  
		...
		status = STM32_RSA_Encrypt(&PubKey_st, Message, sizeof(Message), output);
		...
		if (status == RSA_SUCCESS)
		{
			...
			status = STM32_RSA_Decrypt(&PrivKey_st, output, output, &outputSize);

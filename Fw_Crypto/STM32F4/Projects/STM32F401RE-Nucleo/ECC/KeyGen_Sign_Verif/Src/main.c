    ...
    if (status == HASH_SUCCESS)
    {
      ...
      if (status == ECC_SUCCESS)
      {
        ...
        if (status == ECC_SUCCESS)
        {
          ...
          if (status == ECC_SUCCESS)
          {
            /* Init the random engine and select software random number generator (PRNG) according to the
             * target platform
             */
...
void assert_failed(uint8_t* file, uint32_t line)
{
  ...
  /* Turn on the green led in an infinite loop in case of AES CMAC operations are succssfuls*/
  while (1)
  {}
}
#endif

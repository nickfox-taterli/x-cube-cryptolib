/**
  ******************************************************************************
  * @file    AES/AES128_OFB/Src/main.c
...
int32_t STM32_AES_OFB_Encrypt(uint8_t*  InputMessage,
...
int32_t STM32_AES_OFB_Decrypt(uint8_t*  InputMessage,
                              uint32_t  InputMessageLength,
                              uint8_t  *AES128_Key,
                              uint8_t  *InitializationVector,
                              uint32_t  IvLength,
...
int main(void)
{
  ...
  status = STM32_AES_OFB_Encrypt( (uint8_t *) Plaintext, PLAINTEXT_LENGTH, Key, IV, sizeof(IV), OutputMessage,
                                  &OutputMessageLength);
  ...
  status = STM32_AES_OFB_Decrypt( (uint8_t *) Expected_Ciphertext, PLAINTEXT_LENGTH, Key, IV, sizeof(IV), OutputMessage,
                                  &OutputMessageLength);
  if (status == AES_SUCCESS)
  {
  ...
  /* Turn on the green led in case of AES OFB operations are successful*/
                              ...
                              uint32_t *OutputMessageLength)
{
  ...
  error_status = AES_OFB_Encrypt_Init(&AESctx, AES128_Key, InitializationVector );

  /* check for initialization errors */
  if (error_status == AES_SUCCESS)
  {
    /* Encrypt Data */
    error_status = AES_OFB_Encrypt_Append(&AESctx,
                                          InputMessage,
                                          InputMessageLength,
                                          OutputMessage,
                                          &outputLength);
    ...
    if (error_status == AES_SUCCESS)
    {
      ...
      *OutputMessageLength = outputLength;
      /* Do the Finalization */
      error_status = AES_OFB_Encrypt_Finish(&AESctx, OutputMessage + *OutputMessageLength, &outputLength);
                              ...
                              uint32_t *OutputMessageLength)
{
  ...
  /* Initialize the operation, by passing the key. */
  error_status = AES_OFB_Decrypt_Init(&AESctx, AES128_Key, InitializationVector );

  /* check for initialization errors */
  if (error_status == AES_SUCCESS)
  {
    /* Decrypt Data */
    error_status = AES_OFB_Decrypt_Append(&AESctx,
                                          InputMessage,
                                          InputMessageLength,
                                          OutputMessage,
                                          &outputLength);
    if (error_status == AES_SUCCESS)
    {
      ...
      *OutputMessageLength = outputLength;
      /* Do the Finalization */
      error_status = AES_OFB_Decrypt_Finish(&AESctx, OutputMessage + *OutputMessageLength, &outputLength);
      /* Add data written to the information to be returned */
      *OutputMessageLength += outputLength;
    }
  }

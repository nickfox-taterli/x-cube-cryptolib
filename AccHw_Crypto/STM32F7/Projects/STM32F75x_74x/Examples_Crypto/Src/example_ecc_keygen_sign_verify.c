/**
  ******************************************************************************
  * @file    Examples_Crypto/Src/example_ecc_keygen_sign_verify.c
  * @author  MCD Application Team
  * @version V3.1.0
  * @date    30-October-2015
  * @brief   ECC Key Generation Signature and Verification Example
  ******************************************************************************
  * @attention
  *
...
static int32_t STM32_SHA256_HASH_DigestCompute(uint8_t* InputMessage,
...
int ExampleECCKeyGenSignVerif(void)
{
  ...
  for (param_ecc_counter = 0; param_ecc_counter < 3; param_ecc_counter++)
  {
     ...
     * ECDSA
     */
    status = STM32_SHA256_HASH_DigestCompute((uint8_t*)InputMessage_ECCKeyGenSignVerif,
             InputLength, (uint8_t*)MessageDigest);
      ...
      if (status == ECC_SUCCESS)
      {
        ...
        if (status == ECC_SUCCESS)
        {
          ...
          if (status == ECC_SUCCESS)
          {
            ...
            status = AccHw_ECCkeyGen(PrivKey, PubKey, &EC_st, &Crypto_Buffer);
            ...
            if (status == ECC_SUCCESS)
            {
              ...
              if (status == ECC_SUCCESS)
              {
                ...
                status = AccHw_ECDSAsign(MessageDigest, sizeof(MessageDigest), sign, &signCtx, &Crypto_Buffer);
                if (status == ECC_SUCCESS)
                {
                  ...
                  status = AccHw_ECDSAverify(MessageDigest, sizeof(MessageDigest), sign, &verctx, &Crypto_Buffer);
    ...
    {
      /* if this example runs on STM32F75x devices abd in case of Init the Elliptic Curve is failed.
         Possible return values are: 
       * ECC_ERR_BAD_PARAMETER, ECC_ERR_BAD_CONTEXT, ERR_MEMORY_FAIL, MATH_ERR_INTERNAL
       */

      /* if this example runs on STM32F74x devices abd in case of Init the Elliptic Curve is failed.
          Possible return values are the HAL errors status       
        */
...
static int32_t STM32_SHA256_HASH_DigestCompute(uint8_t* InputMessage, uint32_t InputMessageLength,
    uint8_t *MessageDigest)
{
  int32_t error_status = HAL_OK;
  HASH_HandleTypeDef     HashHandle;

  error_status = HAL_HASH_DeInit(&HashHandle);
  if (error_status == HAL_OK)
  {
    HashHandle.Init.DataType = HASH_DATATYPE_8B;
    error_status = HAL_HASH_Init(&HashHandle);
    if (error_status == HAL_OK)
    {
      error_status = HAL_HASHEx_SHA256_Start(&HashHandle, InputMessage, InputMessageLength, MessageDigest, 0xFF);
      if (error_status == HAL_OK)
      {
        error_status = HAL_HASHEx_SHA256_Finish(&HashHandle, MessageDigest, 0xFF);
      }
